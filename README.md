...
data[0];int const stride =size / 2; 
...
// unroll4 + complete unroll for loop + gmem
 ...
 global void reduceGmem(int *g_idata, int *g_odata, unsigned int n)
{
...
idata[0];
 ...
 global void reduceSmem(int *g_idata, int *g_odata, unsigned int n)
{
...
// convert global data pointer to the local pointer of this blockint
*idata = g_idata + blockIdx.x * blockDim.x;
...
vsmem[tid] += vsmem[tid +
32];vsmem[tid] += vsmem[tid +
16];[tid] += vsmem[tid
+ 8]; vsmem[tid] += vsmem[tid
+ 4]; vsmem[tid] += vsmem[tid
+ 2]; vsmem[tid] += vsmem[tid
+ 1];
}
// write result for this block to global mem
if (tid == 0) g_odata[blockIdx.x] =
smem[0];
 ...
 global void reduceSmemDyn(int *g_idata, int *g_odata, unsigned int n)
...
// unroll4 + complete unroll for loop + gmem
 global void reduceGmemUnroll(int *g_idata, int *g_odata, unsigned int n)
{
...
// convert global data pointer to the local pointer of this blockint
*idata = g_idata + blockIdx.x * blockDim.x * 4;
// unrolling 4
...
if (idx + 2 * blockDim.x < n) a3 = g_idata[idx + 2 *
blockDim.x];if (idx + 3 * blockDim.x < n) a4 = g_idata[idx +
3 * blockDim.x];g_idata[idx] = a1 + a2 + a3 + a4;
}
 syncthreads();
// in-place reduction in global memory
...
idata[0];
 ...
 global void reduceSmemUnroll(int *g_idata, int *g_odata, unsigned int n)
{
...
smem[0];
 ...
 global void reduceSmemUnrollDyn(int *g_idata, int *g_odata, unsigned int n)
{
...
if (idx + 2 * blockDim.x < n) a3 = g_idata[idx + 2 *
blockDim.x];if (idx + 3 * blockDim.x < n) a4 = g_idata[idx +
3 * blockDim.x];tmpSum = a1 + a2 + a3 + a4;
}
smem[tid] = tmpSum;
...
smem[0];
 ...
 global void reduceNeighboredGmem(int *g_idata, int *g_odata, unsigned int n)
{
...
idata[0];
 ...
 global void reduceNeighboredSmem(int *g_idata, int *g_odata, unsigned int n)
{
 shared int smem[DIM];
// set thread ID
unsigned int tid = threadIdx.x;
...
int *h_odata = (int *) malloc(grid.x *
sizeof(int));int *tmp = (int *) malloc(bytes);
...
OUTPUT:
reduceInteger8.cu:
root@SAV-MLSystem:/home/student/Exp_3# nvcc reduceInteger8.cu -o reduceInteger8

root@SAV-MLSystem:/home/student/Exp_3# nvcc reduceInteger8.cu

root@SAV-MLSystem:/home/student/Exp_3# ./reduceInteger8
...
Test failed!
reduceInteger16.cu:
root@SAV-MLSystem:/home/student/exp_3# nvcc reduceInteger16.cu -o reduceInteger16

root@SAV-MLSystem:/home/student/exp_3# nvcc reduceInteger16.cu

root@SAV-MLSystem:/home/student/exp_3# ./reduceInteger16
...
reduceSmemDynUnroll16: 133554970 <<<grid 2048 block
